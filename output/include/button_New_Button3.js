...
Runner.buttonEvents["New_Button3"]=function(pageObj,proxy,pageid){pageObj.buttonNames[pageObj.buttonNames.length]='New_Button3';if(!pageObj.buttonEventBefore['New_Button3']){pageObj.buttonEventBefore['New_Button3']=function(params,ctrl,pageObj,proxy,pageid,rowData,row,submit){var ajax=ctrl;params["ProcesoId"]=834954;params["OficioId"]=row.getFieldValue("OficioId");obligacion=$.ajax({type:"method",url:"consultas.php?w1="+params["ProcesoId"],dataType:'text',async:false,success:function(response){console.log("enviado valor de select por GET");}}).responseText;console.log("Obligacion: "+obligacion);obligacion=Number(obligacion);function Unidades(num){switch(num)
