...
Runner.buttonEvents["New_Button3"]=function(pageObj,proxy,pageid){pageObj.buttonNames[pageObj.buttonNames.length]='New_Button3';if(!pageObj.buttonEventBefore['New_Button3']){pageObj.buttonEventBefore['New_Button3']=function(params,ctrl,pageObj,proxy,pageid,rowData,row,submit){var ajax=ctrl;params["ProcesoId"]=row.getFieldValue("ProcesoId");params["OficioId"]=row.getFieldValue("OficioId");obligacion=$.ajax({type:"method",url:"consultas.php?w1="+params["ProcesoId"],dataType:'text',async:false,success:function(response){console.log("enviado valor de select por GET");}}).responseText;obligacionTotal=$.ajax({type:"method",url:"consultas.php?w2="+params["ProcesoId"],dataType:'text',async:false,success:function(response){console.log("enviado valor de select por GET");}}).responseText;obligacion=Number(obligacion);obligacionTotal=Number(obligacionTotal);function Unidades(num){switch(num)
