...
Runner.buttonEvents["ImprimirOfChe"]=function(pageObj,proxy,pageid){pageObj.buttonNames[pageObj.buttonNames.length]='ImprimirOfChe';if(!pageObj.buttonEventBefore['ImprimirOfChe']){pageObj.buttonEventBefore['ImprimirOfChe']=function(params,ctrl,pageObj,proxy,pageid,rowData,row,submit){var ajax=ctrl;params["ProcesoId"]=row.getFieldValue("ProcesoId");params["OficioId"]=row.getFieldValue("OficioId");obligacion=$.ajax({type:"method",url:"consultas.php?w4="+params["ChequeoId"],dataType:'text',async:false,success:function(response){console.log("enviado valor de select por GET");}}).responseText;obligacionTotal=$.ajax({type:"method",url:"consultas.php?w4="+params["ChequeoId"],dataType:'text',async:false,success:function(response){console.log("enviado valor de select por GET");}}).responseText;obligacion=Number(obligacion);obligacionTotal=Number(obligacionTotal);function Unidades(num){switch(num)
