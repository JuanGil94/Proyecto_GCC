...
if(!pageObj.buttonEventAfter['New_Button']){pageObj.buttonEventAfter['New_Button']=function(result,ctrl,pageObj,proxy,pageid,rowData,row,params){var ajax=ctrl;if(result["Dev"]===1){console.log("Entro");swal("Warning","No se puede crear el Proceso si esta en tramite Devolucion","warning");}
else if(result["total"]){swal({icon:"success",text:"El Chequeo "+params["ChequeoId"]+" contiene Sancionado. Se crea el proceso Correctamente"}).then(function(value){switch(value){default:location.reload();break;}});}
