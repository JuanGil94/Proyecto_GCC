...
function procesarSeleccion1() {
    var select = document.getElementById("numeroSeleccionado"); // Obtiene el elemento select
    var numero = select.value;

    if (numero !== "") {
        if (document.getElementById('complementoActivo').value == '0') {
            concatenaNumero(document.getElementById('puente').value, numero, 'puente');
            concatenaNumero(document.getElementById('direccion').value, numero, 'direccion');
        } else {
            enComplemento();
        }

        // Restablecer la selección después de un breve tiempo para que la acción se complete
        setTimeout(() => { select.value = ""; }, 200);
    }
}

document.getElementById("letras").addEventListener("change", function() {
    const select = this;
    const letra = select.value;

    if (letra) {
        if (document.getElementById('complementoActivo')?.value == '0') {
            concatenaString(document.getElementById('puente')?.value, letra, 'puente');
            concatenaString(document.getElementById('direccion')?.value, letra, 'direccion');
        } else {
            enComplemento();
        }
        ...
        var direccionPrueba = document.getElementById("direccion_prueba");

        if (direccionPrueba && direccionPrueba.value !== direccion) {
            direccionPrueba.value = direccion;
        }

    function seleccionarcomplemento(valor) {
    document.getElementById("complementocalle").value = valor;
    procesarSeleccion1();  // Llamar a tu función original 
    //document.getElementById("calculadora").style.display = "none"; // Ocultar después de seleccionar
    }
  ...
  function seleccionarValor(valor) {
    document.getElementById("numeroSeleccionado").value = valor;
    procesarSeleccion1();  // Llamar a tu función original 
    //document.getElementById("calculadora").style.display = "none"; // Ocultar después de seleccionar
  }


  // Cerrar la calculadora si se hace clic fuera
  document.addEventListener("click", function (event) {
    let calcContainer = document.querySelector(".calculadora-container");
    if (!calcContainer.contains(event.target)) {
