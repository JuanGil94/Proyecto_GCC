...
function recaudo($procesoId){
    ...
    //$obliPor=($obliInicial/($obliInicial+$costas+$inteAcu));
    //$intePor=($inteAcu/($obliInicial+$costas+$inteAcu));

}
function insertRe($procesoId,$fecha,$dias,$tasa,$intereses,$obliReca,$obliNove,$obliSald,$inteReca,$inteNove,$inteSald,$costReca,$costNove,$costSald){
    try{
...
$recaudos=recaudo($numero);
$array=array();
$array2=array();
$array3=array();
$consulta=DB::Query("SELECT * FROM Procesos WHERE ProcesoId=".$numero);
while($date=$consulta->fetchAssoc()){
    //echo $date["Ejecutoria"]."<br>";
		$params["Numero"]=$date["Numero"];
    $naturaleza=$date["NaturalezaId"];
    $costas=$date["CostasInicial"];
    $concepto=$date["ConceptoId"];
    $obligacion=$date["ObligacionInicial"];
    if (!$date["Plazo"]){
        $date["Plazo"]=$date["Ejecutoria"];
    }
    $fechaP=$date["Plazo"];//echo "la fecha plazo es: ".$date["Plazo"];
}
...
for ($i=0;$i<=$numAnnos;$i++){
    ...
    foreach ($meses as $mes => $dias) {
        ...
        if ($annoEje==$annoHasta && ($mesDesde<$mes&&$mes<$mesHasta)){
            ...
            insertRe($numero,$fechaBase,$dias,$tasaDiaraT,$valorDias,$obliReca,$obliNove,$obligacion,$inteReca,$inteNove,$sumaTotal,$costReca,$costNove,$costSald);  
        }
            ...
            foreach($elementos as $dia){
                ...
                foreach ($recaudos as $fecha) {
                    if($fecha["Fecha"]==$fechaCom) {
                        ...
                        insertRe($numero,$fechaIns,$dia,$tasaDiaraT,$valorCorte,$obliReca,$obliNove,$obligacion,$inteReca,$inteNove,$sumaTotalDiaria,$costReca,$costNove,$costSald);
                        $obligacion=$obligacion-$obliPor;
                        $valorDiario=round(($tasaDiaraT*$obligacion*100),2);
            ...
            insertRe($numero,$fechaBase,$dia,$tasaDiaraT,$valorDias,$obliReca,$obliNove,$obligacion,$inteReca,$inteNove,$sumaTotal,$costReca,$costNove,$costSald);  
        }
            ...
            for($dia=1;$dia<=$numDiasMesAct;$dia++){ 
                ...
                foreach ($recaudos as $fecha) {
                    if($fecha["Fecha"]==$fechaCom) {
                        ...
                        insertRe($numero,$fechaCom,$dia,$tasaDiaraT,$valorCorte,$obliReca,$obliNove,$obligacion,$inteReca,$inteNove,$sumaTotalDiaria,$costReca,$costNove,$costSald);   
                    }
                    //echo "La Fecha del recaudo fue: ".$fecha["Fecha"]." y el valor fue de: ".$fecha["Pago"]."<br>";
                }
            }
            ...
            if ($numDiasMesAct>0){
                insertRe($numero,$fechaBase,$numDiasMesAct,$tasaDiaraT,$valorDias,$obliReca,$obliNove,$obligacion,$inteReca,$inteNove,$sumaTotal,$costReca,$costNove,$costSald);
            }
         }
        else if ($mesEje>$mes && $annobase==$annoEje){
            ...
            $tasaDiaraT=tasa($naturaleza,$concepto,$annoEje,$mes);
